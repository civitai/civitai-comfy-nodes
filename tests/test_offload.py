...
def _selector_node(by_slot):
    return {"class_type": "CivitaiModelSelector", "inputs": {"resources_json": json.dumps({"bySlot": by_slot})}}


def test_bake_model_selectors_swaps_slot_links_and_removes_selector():
    prompt = {
        "10": _selector_node({"1": "urn:air:sdxl:checkpoint:civitai:1@2", "3": "urn:air:sdxl:vae:civitai:3@4"}),
        "20": {"class_type": "CheckpointLoaderSimple", "inputs": {"ckpt_name": ["10", 1]}},
        "21": {"class_type": "VAELoader", "inputs": {"vae_name": ["10", 3]}},
    }
    resources = set()
    left = offload.bake_model_selectors(prompt, resources)
    assert left == []
    assert "10" not in prompt  # fully baked → selector deleted, never runs on the worker
    assert prompt["20"]["inputs"]["ckpt_name"] == "urn:air:sdxl:checkpoint:civitai:1@2"
    assert prompt["21"]["inputs"]["vae_name"] == "urn:air:sdxl:vae:civitai:3@4"
    assert resources == {"urn:air:sdxl:checkpoint:civitai:1@2", "urn:air:sdxl:vae:civitai:3@4"}


def test_bake_model_selectors_leaves_selector_when_air_output_wired():
    # A downstream node consuming slot 0 (the `air` output) has no file-pinned AIR, so the selector
    # must stay in the graph (and be reported) rather than being deleted.
    prompt = {
        "10": _selector_node({"1": "urn:air:sdxl:checkpoint:civitai:1@2"}),
        "20": {"class_type": "CheckpointLoaderSimple", "inputs": {"ckpt_name": ["10", 1]}},
        "22": {"class_type": "SomeRecipe", "inputs": {"model": ["10", 0]}},
    }
    resources = set()
    left = offload.bake_model_selectors(prompt, resources)
    assert left == ["10"]
    assert "10" in prompt
    assert prompt["20"]["inputs"]["ckpt_name"] == "urn:air:sdxl:checkpoint:civitai:1@2"
    assert prompt["22"]["inputs"]["model"] == ["10", 0]  # slot-0 link untouched


def test_build_custom_comfy_offload_bakes_model_selector_into_resources():
    prompt = {
        "10": _selector_node({"1": "urn:air:sdxl:checkpoint:civitai:1@2"}),
        "20": {"class_type": "CheckpointLoaderSimple", "inputs": {"ckpt_name": ["10", 1]}},
    ...
    workflow = built.steps[0]["input"]["workflow"]
    assert "10" not in workflow
    assert workflow["20"]["inputs"]["ckpt_name"] == "urn:air:sdxl:checkpoint:civitai:1@2"
