...
- `civitai_comfy_nodes/link.py` + `link_protocol.py` — Civitai Link client. `link_protocol` is
  ...
  `local_models.stream_download`; the installed list comes from `offload.scan_local_model_files`
  + `model_cache`, with `offload.resolve_model_air` filling missing hashes in the background.
...
- A safetensors header can't contain its own whole-file SHA256; header hashes are AutoV3-style
  and resolve to the real SHA256 through the by-hash API (`offload._with_canonical_sha256`).
