...
from . import config, local_models, model_cache, offload
...
def _scan_local_models():
    return offload.scan_local_model_files(offload.model_roots_by_folder(proto.LIST_FOLDERS))
...
class LinkClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._hash_lookup = hash_lookup or offload.lookup_model_version_by_hash
        self._resolver = resolver or offload.resolve_model_air
        self._token_provider = token_provider or (lambda: config.auth_state()[0])
        self._scan = scan or _scan_local_models
        self._folder_roots = folder_roots or offload._folder_paths_for
        self._sio = None
    ...
    def _run_add(self, command: dict, cancel: threading.Event) -> None:
        ...
        file = offload.version_file_for_hash(version, expected)
