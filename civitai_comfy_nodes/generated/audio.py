# AUTO-GENERATED by codegen/generate.py from spec/v2-consumers.json (sha256: 1f3579b07645).
# DO NOT EDIT — adjust codegen/overrides.json and re-run scripts/sync-spec.sh instead.

from ..base import CivitaiRecipeNodeBase, F, O


class CivitaiAceStepAudio(CivitaiRecipeNodeBase):
    """Civitai Ace Step Audio — aceStepAudio recipe via Civitai Orchestration."""

    RECIPE = "aceStepAudio"
    STEP_TYPE = "aceStepAudio"
    CATEGORY = "Civitai/Audio"
    FUNCTION = "run"
    RETURN_TYPES = ("AUDIO", "VIDEO", "STRING", "STRING")
    RETURN_NAMES = ("blob_audio", "blob_video", "workflow_id", "raw_json")
    FIELDS = {
        "music_description": F("musicDescription", "value"),
        "lyrics": F("lyrics", "value"),
        "seed": F("seed", "value"),
        "duration": F("duration", "value"),
        "bpm": F("bpm", "value"),
        "time_signature": F("timeSignature", "value"),
        "language": F("language", "value"),
        "key": F("key", "value"),
        "instrumental_weight": F("instrumentalWeight", "value"),
        "vocal_weight": F("vocalWeight", "value"),
        "diffusion_model": F("diffusionModel", "air"),
        "cover_json": F("cover", "json"),
        "clip_model": F("clipModel", "air"),
        "vae_model": F("vaeModel", "air"),
        "language_model": F("languageModel", "air"),
        "steps": F("steps", "value"),
        "cfg": F("cfg", "value"),
        "loras": F("loras", "lora_strength_map"),
    }
    OUTPUTS = (O("blob", "audio_or_video"),)

    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "music_description": (
                    "STRING",
                    {
                        "tooltip": 'Music style/genre description (e.g., "Neo-Soul: A warm, organic neo-soul track...")',
                        "default": "",
                        "multiline": True,
                    },
                ),
                "lyrics": (
                    "STRING",
                    {
                        "tooltip": "Structured lyrics with section markers like [Verse], [Chorus], [Bridge], etc.",
                        "default": "",
                        "multiline": True,
                    },
                ),
                "seed": (
                    "INT",
                    {
                        "tooltip": "Random seed for reproducible generation",
                        "control_after_generate": True,
                        "default": 0,
                        "min": 0,
                        "max": 4294967295,
                        "step": 1,
                    },
                ),
            },
            "optional": {
                "duration": (
                    "FLOAT",
                    {"tooltip": "Duration in seconds (1-190)", "default": 60.0, "min": 1.0, "max": 190.0, "step": 0.01},
                ),
                "bpm": (
                    "INT",
                    {"tooltip": "Beats per minute (40-200)", "default": 120, "min": 40, "max": 200, "step": 1},
                ),
                "time_signature": (
                    ["2", "3", "4", "6"],
                    {"tooltip": 'Time signature (e.g., "4" for 4/4 time)', "default": "4"},
                ),
                "language": (
                    [
                        "en",
                        "ja",
                        "zh",
                        "es",
                        "de",
                        "fr",
                        "pt",
                        "ru",
                        "it",
                        "nl",
                        "pl",
                        "tr",
                        "vi",
                        "cs",
                        "fa",
                        "id",
                        "ko",
                        "uk",
                        "hu",
                        "ar",
                        "sv",
                        "ro",
                        "el",
                    ],
                    {"tooltip": 'Language code (e.g., "en", "zh", "ja", "ko")', "default": "en"},
                ),
                "key": (
                    [
                        "C major",
                        "C# major",
                        "Db major",
                        "D major",
                        "D# major",
                        "Eb major",
                        "E major",
                        "F major",
                        "F# major",
                        "Gb major",
                        "G major",
                        "G# major",
                        "Ab major",
                        "A major",
                        "A# major",
                        "Bb major",
                        "B major",
                        "C minor",
                        "C# minor",
                        "Db minor",
                        "D minor",
                        "D# minor",
                        "Eb minor",
                        "E minor",
                        "F minor",
                        "F# minor",
                        "Gb minor",
                        "G minor",
                        "G# minor",
                        "Ab minor",
                        "A minor",
                        "A# minor",
                        "Bb minor",
                        "B minor",
                    ],
                    {
                        "tooltip": 'Musical key (e.g., "C major", "E minor"). Mirrors ComfyUI\'s TextEncodeAceStepAudio1.5 keyscale combo: 17 roots × {major, minor}.',
                        "default": "C major",
                    },
                ),
                "instrumental_weight": (
                    "FLOAT",
                    {
                        "tooltip": "Weight for instrumental elements (0.0-1.0)",
                        "default": 0.85,
                        "min": 0.0,
                        "max": 2147483647.0,
                        "step": 0.01,
                    },
                ),
                "vocal_weight": (
                    "FLOAT",
                    {
                        "tooltip": "Weight for vocal elements (0.0-1.0)",
                        "default": 0.9,
                        "min": 0.0,
                        "max": 2147483647.0,
                        "step": 0.01,
                    },
                ),
                "diffusion_model": (
                    "CIVITAI_AIR",
                    {"tooltip": "Optional diffusion model (unet) override. Defaults to ACE-Step 1.5 turbo 2B."},
                ),
                "cover_json": (
                    "STRING",
                    {
                        "tooltip": "Cover image configuration for ACE Step audio output. When present, the output is a WebM video with this image as the visual.",
                        "default": "",
                        "multiline": True,
                    },
                ),
                "clip_model": (
                    "CIVITAI_AIR",
                    {"tooltip": "Optional text encoder (CLIP) override. Defaults to qwen_4b_ace15."},
                ),
                "vae_model": ("CIVITAI_AIR", {"tooltip": "Optional VAE override. Defaults to ace_1.5_vae."}),
                "language_model": (
                    "CIVITAI_AIR",
                    {
                        "tooltip": "Optional language model override for audio code generation. Defaults to qwen_0.6b_ace15."
                    },
                ),
                "steps": (
                    "INT",
                    {
                        "tooltip": "Number of sampling steps. Turbo variants converge at 8; non-turbo base / sft variants expect the full 50-step schedule.",
                        "default": 8,
                        "min": 1,
                        "max": 200,
                        "step": 1,
                    },
                ),
                "cfg": (
                    "FLOAT",
                    {
                        "tooltip": "Classifier-free guidance scale. Turbo variants run CFG-off at 1.0; non-turbo base / sft variants expect CFG on (around 4).",
                        "default": 1.0,
                        "min": 0.0,
                        "max": 30.0,
                        "step": 0.01,
                    },
                ),
                "loras": (
                    "CIVITAI_LORAS",
                    {
                        "tooltip": "Optional LoRAs to apply. Each entry's strength is applied to both the diffusion model (UNET) and the dual CLIP via ComfyUI's LoraLoader. Compatibility with non-default base models is the caller's responsibility."
                    },
                ),
                "api_config": (
                    "CIVITAI_CONFIG",
                    {
                        "tooltip": "Optional Civitai Auth connection; defaults to CIVITAI_API_TOKEN or stored OAuth login."
                    },
                ),
            },
        }


class CivitaiAudioCaptioning(CivitaiRecipeNodeBase):
    """Civitai Audio Captioning — audioCaptioning recipe via Civitai Orchestration."""

    RECIPE = "audioCaptioning"
    STEP_TYPE = "audioCaptioning"
    CATEGORY = "Civitai/Audio"
    FUNCTION = "run"
    RETURN_TYPES = ("STRING", "STRING", "STRING")
    RETURN_NAMES = ("results", "workflow_id", "raw_json")
    FIELDS = {
        "media_url": F("mediaUrl", "audio_url"),
        "temperature": F("temperature", "value"),
        "max_new_tokens": F("maxNewTokens", "value"),
    }
    OUTPUTS = (O("results", "json"),)

    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "media_url": ("AUDIO", {"tooltip": "The URL of the audio file or zip archive to caption."}),
                "temperature": (
                    "FLOAT",
                    {
                        "tooltip": "Sampling temperature for both ACE-Step requests.",
                        "default": 0.0,
                        "min": 0.0,
                        "max": 2147483647.0,
                        "step": 0.01,
                    },
                ),
                "max_new_tokens": (
                    "INT",
                    {
                        "tooltip": "Maximum number of tokens to generate for each ACE-Step request.",
                        "default": 0,
                        "min": 0,
                        "max": 2147483647,
                        "step": 1,
                    },
                ),
            },
            "optional": {
                "api_config": (
                    "CIVITAI_CONFIG",
                    {
                        "tooltip": "Optional Civitai Auth connection; defaults to CIVITAI_API_TOKEN or stored OAuth login."
                    },
                ),
            },
        }


class CivitaiMiniMaxMusic3(CivitaiRecipeNodeBase):
    """Civitai MiniMax Music 3 — miniMaxMusic3 recipe via Civitai Orchestration."""

    RECIPE = "miniMaxMusic3"
    STEP_TYPE = "miniMaxMusic3"
    CATEGORY = "Civitai/Audio"
    FUNCTION = "run"
    RETURN_TYPES = ("AUDIO", "STRING", "STRING")
    RETURN_NAMES = ("blob", "workflow_id", "raw_json")
    FIELDS = {
        "caption": F("caption", "value"),
        "lyrics": F("lyrics", "value"),
        "seed": F("seed", "value"),
        "max_duration": F("maxDuration", "value"),
        "steps": F("steps", "value"),
        "cfg": F("cfg", "value"),
        "top_k": F("topK", "value"),
        "diffusion_model": F("diffusionModel", "air"),
        "text_encoder": F("textEncoder", "air"),
        "vae": F("vae", "air"),
        "loras": F("loras", "lora_strength_map"),
    }
    OUTPUTS = (O("blob", "audio"),)

    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "caption": (
                    "STRING",
                    {
                        "tooltip": "Structured music description. For best results use Global Metadata, Vocal Details, and Arrangement sections.",
                        "default": "",
                    },
                ),
                "lyrics": (
                    "STRING",
                    {
                        "tooltip": "Lyrics with section markers such as [Intro], [Verse], [Chorus], and [Outro].",
                        "default": "",
                        "multiline": True,
                    },
                ),
                "seed": (
                    "INT",
                    {
                        "tooltip": "Random seed for reproducible generation.",
                        "control_after_generate": True,
                        "default": 0,
                        "min": 0,
                        "max": 4294967295,
                        "step": 1,
                    },
                ),
            },
            "optional": {
                "max_duration": (
                    "FLOAT",
                    {
                        "tooltip": "Maximum generated song duration in seconds. The model may end the song earlier when the requested lyric structure is complete.",
                        "default": 120.0,
                        "min": 0.03999999910593033,
                        "max": 360.0,
                        "step": 0.01,
                    },
                ),
                "steps": (
                    "INT",
                    {"tooltip": "Number of diffusion sampling steps.", "default": 30, "min": 1, "max": 100, "step": 1},
                ),
                "cfg": (
                    "FLOAT",
                    {
                        "tooltip": "Classifier-free guidance scale used by both text encoding and sampling.",
                        "default": 1.7,
                        "min": 0.0,
                        "max": 100.0,
                        "step": 0.01,
                    },
                ),
                "top_k": (
                    "INT",
                    {
                        "tooltip": "Top-k sampling limit used by the MiniMax music text encoder.",
                        "default": 50,
                        "min": 1,
                        "max": 16384,
                        "step": 1,
                    },
                ),
                "diffusion_model": ("CIVITAI_AIR", {"tooltip": "Optional diffusion model override."}),
                "text_encoder": ("CIVITAI_AIR", {"tooltip": "Optional MiniMax text encoder override."}),
                "vae": ("CIVITAI_AIR", {"tooltip": "Optional audio VAE override."}),
                "loras": (
                    "CIVITAI_LORAS",
                    {
                        "tooltip": "Optional LoRAs to apply to both the diffusion model and MiniMax text encoder. Compatibility with the selected base resources is the caller's responsibility."
                    },
                ),
                "api_config": (
                    "CIVITAI_CONFIG",
                    {
                        "tooltip": "Optional Civitai Auth connection; defaults to CIVITAI_API_TOKEN or stored OAuth login."
                    },
                ),
            },
        }


class CivitaiTextToSpeechVllmOmniQwen3Base(CivitaiRecipeNodeBase):
    """qwen3 / base — textToSpeech recipe via Civitai Orchestration."""

    RECIPE = "textToSpeech"
    STEP_TYPE = "textToSpeech"
    DISCRIMINATOR = {"engine": "vllm-omni", "ecosystem": "qwen3", "operation": "base"}
    CATEGORY = "Civitai/Audio/qwen3"
    FUNCTION = "run"
    RETURN_TYPES = ("AUDIO", "STRING", "STRING", "STRING", "STRING")
    RETURN_NAMES = ("audio_blob", "model_type", "speaker", "workflow_id", "raw_json")
    FIELDS = {
        "text": F("text", "value"),
        "language": F("language", "value"),
        "max_new_tokens": F("maxNewTokens", "value"),
        "ref_audio_url": F("refAudioUrl", "audio_url"),
        "ref_text": F("refText", "value"),
        "x_vector_only_mode": F("xVectorOnlyMode", "value"),
    }
    OUTPUTS = (
        O("audioBlob", "audio"),
        O("modelType", "string"),
        O("speaker", "string"),
    )

    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "text": (
                    "STRING",
                    {"tooltip": "The text to synthesize into speech.", "default": "", "multiline": True},
                ),
                "x_vector_only_mode": (
                    "BOOLEAN",
                    {"tooltip": "If true, uses only speaker embedding (ref_text not required).", "default": False},
                ),
            },
            "optional": {
                "language": (
                    "STRING",
                    {"tooltip": 'Target language (e.g., "English", "Chinese"). Defaults to "Auto".', "default": ""},
                ),
                "max_new_tokens": (
                    "INT",
                    {
                        "tooltip": "Optional generation cap for max tokens.",
                        "default": 0,
                        "min": 0,
                        "max": 2147483647,
                        "step": 1,
                    },
                ),
                "ref_audio_url": (
                    "AUDIO",
                    {
                        "tooltip": "Reference audio AIR URN or external URL for voice cloning. Accepts AIR URNs (existing resources) or HTTP(S) URLs."
                    },
                ),
                "ref_text": (
                    "STRING",
                    {
                        "tooltip": "Transcript of the reference audio. Required for Base mode unless XVectorOnlyMode is true.",
                        "default": "",
                    },
                ),
                "api_config": (
                    "CIVITAI_CONFIG",
                    {
                        "tooltip": "Optional Civitai Auth connection; defaults to CIVITAI_API_TOKEN or stored OAuth login."
                    },
                ),
            },
        }


class CivitaiTextToSpeechVllmOmniQwen3CustomVoice(CivitaiRecipeNodeBase):
    """qwen3 / customVoice — textToSpeech recipe via Civitai Orchestration."""

    RECIPE = "textToSpeech"
    STEP_TYPE = "textToSpeech"
    DISCRIMINATOR = {"engine": "vllm-omni", "ecosystem": "qwen3", "operation": "customVoice"}
    CATEGORY = "Civitai/Audio/qwen3"
    FUNCTION = "run"
    RETURN_TYPES = ("AUDIO", "STRING", "STRING", "STRING", "STRING")
    RETURN_NAMES = ("audio_blob", "model_type", "speaker", "workflow_id", "raw_json")
    FIELDS = {
        "text": F("text", "value"),
        "language": F("language", "value"),
        "max_new_tokens": F("maxNewTokens", "value"),
        "speaker": F("speaker", "value"),
        "instruct": F("instruct", "value"),
    }
    OUTPUTS = (
        O("audioBlob", "audio"),
        O("modelType", "string"),
        O("speaker", "string"),
    )

    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "text": (
                    "STRING",
                    {"tooltip": "The text to synthesize into speech.", "default": "", "multiline": True},
                ),
                "speaker": (
                    ["aiden", "dylan", "eric", "ono_anna", "ryan", "serena", "sohee", "uncle_fu", "vivian"],
                    {"tooltip": "Built-in speaker name for CustomVoice mode."},
                ),
            },
            "optional": {
                "language": (
                    "STRING",
                    {"tooltip": 'Target language (e.g., "English", "Chinese"). Defaults to "Auto".', "default": ""},
                ),
                "max_new_tokens": (
                    "INT",
                    {
                        "tooltip": "Optional generation cap for max tokens.",
                        "default": 0,
                        "min": 0,
                        "max": 2147483647,
                        "step": 1,
                    },
                ),
                "instruct": (
                    "STRING",
                    {"tooltip": 'Optional style instruction (e.g., "speak slowly and clearly").', "default": ""},
                ),
                "api_config": (
                    "CIVITAI_CONFIG",
                    {
                        "tooltip": "Optional Civitai Auth connection; defaults to CIVITAI_API_TOKEN or stored OAuth login."
                    },
                ),
            },
        }


class CivitaiTextToSpeechVllmOmniQwen3VoiceDesign(CivitaiRecipeNodeBase):
    """qwen3 / voiceDesign — textToSpeech recipe via Civitai Orchestration."""

    RECIPE = "textToSpeech"
    STEP_TYPE = "textToSpeech"
    DISCRIMINATOR = {"engine": "vllm-omni", "ecosystem": "qwen3", "operation": "voiceDesign"}
    CATEGORY = "Civitai/Audio/qwen3"
    FUNCTION = "run"
    RETURN_TYPES = ("AUDIO", "STRING", "STRING", "STRING", "STRING")
    RETURN_NAMES = ("audio_blob", "model_type", "speaker", "workflow_id", "raw_json")
    FIELDS = {
        "text": F("text", "value"),
        "language": F("language", "value"),
        "max_new_tokens": F("maxNewTokens", "value"),
        "instruct": F("instruct", "value"),
    }
    OUTPUTS = (
        O("audioBlob", "audio"),
        O("modelType", "string"),
        O("speaker", "string"),
    )

    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "text": (
                    "STRING",
                    {"tooltip": "The text to synthesize into speech.", "default": "", "multiline": True},
                ),
                "instruct": (
                    "STRING",
                    {
                        "tooltip": 'Natural-language description of the desired voice (e.g., "a calm middle-aged male narrator with a slight British accent").',
                        "default": "",
                    },
                ),
            },
            "optional": {
                "language": (
                    "STRING",
                    {"tooltip": 'Target language (e.g., "English", "Chinese"). Defaults to "Auto".', "default": ""},
                ),
                "max_new_tokens": (
                    "INT",
                    {
                        "tooltip": "Optional generation cap for max tokens.",
                        "default": 0,
                        "min": 0,
                        "max": 2147483647,
                        "step": 1,
                    },
                ),
                "api_config": (
                    "CIVITAI_CONFIG",
                    {
                        "tooltip": "Optional Civitai Auth connection; defaults to CIVITAI_API_TOKEN or stored OAuth login."
                    },
                ),
            },
        }


class CivitaiTextToSpeechVllmOmniOmnivoice(CivitaiRecipeNodeBase):
    """omnivoice — textToSpeech recipe via Civitai Orchestration."""

    RECIPE = "textToSpeech"
    STEP_TYPE = "textToSpeech"
    DISCRIMINATOR = {"engine": "vllm-omni", "ecosystem": "omnivoice"}
    CATEGORY = "Civitai/Audio/omnivoice"
    FUNCTION = "run"
    RETURN_TYPES = ("AUDIO", "STRING", "STRING", "STRING", "STRING")
    RETURN_NAMES = ("audio_blob", "model_type", "speaker", "workflow_id", "raw_json")
    FIELDS = {
        "text": F("text", "value"),
        "language": F("language", "value"),
        "ref_audio_url": F("refAudioUrl", "audio_url"),
        "ref_text": F("refText", "value"),
        "instruct": F("instruct", "value"),
    }
    OUTPUTS = (
        O("audioBlob", "audio"),
        O("modelType", "string"),
        O("speaker", "string"),
    )

    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "text": (
                    "STRING",
                    {"tooltip": "The text to synthesize into speech.", "default": "", "multiline": True},
                ),
            },
            "optional": {
                "language": (
                    "STRING",
                    {"tooltip": 'Target language (e.g., "English", "Chinese"). Defaults to "Auto".', "default": ""},
                ),
                "ref_audio_url": ("AUDIO", {"tooltip": "Reference audio AIR URN or external URL for voice cloning."}),
                "ref_text": ("STRING", {"tooltip": "Transcript of the reference audio.", "default": ""}),
                "instruct": ("STRING", {"tooltip": "Optional voice/style instruction.", "default": ""}),
                "api_config": (
                    "CIVITAI_CONFIG",
                    {
                        "tooltip": "Optional Civitai Auth connection; defaults to CIVITAI_API_TOKEN or stored OAuth login."
                    },
                ),
            },
        }


class CivitaiTranscription(CivitaiRecipeNodeBase):
    """Civitai Transcription — transcription recipe via Civitai Orchestration."""

    RECIPE = "transcription"
    STEP_TYPE = "transcription"
    CATEGORY = "Civitai/Audio"
    FUNCTION = "run"
    RETURN_TYPES = ("STRING", "STRING", "STRING", "STRING", "STRING", "STRING")
    RETURN_NAMES = ("text", "language", "time_stamps", "elapsed_seconds", "workflow_id", "raw_json")
    FIELDS = {
        "media_url": F("mediaUrl", "audio_url"),
        "language": F("language", "value"),
        "context": F("context", "value"),
        "return_time_stamps": F("returnTimeStamps", "value"),
    }
    OUTPUTS = (
        O("text", "string"),
        O("language", "string"),
        O("timeStamps", "json"),
        O("elapsedSeconds", "string"),
    )

    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "media_url": ("AUDIO", {"tooltip": "The URL of the audio media to transcribe."}),
            },
            "optional": {
                "language": (
                    "STRING",
                    {"tooltip": 'Optional language hint (e.g., "en", "zh") to guide transcription.', "default": ""},
                ),
                "context": (
                    "STRING",
                    {
                        "tooltip": "Optional context prompt to improve transcription accuracy.",
                        "default": "",
                        "multiline": True,
                    },
                ),
                "return_time_stamps": (
                    "BOOLEAN",
                    {"tooltip": "Whether to return word-level timestamps. Defaults to true.", "default": True},
                ),
                "api_config": (
                    "CIVITAI_CONFIG",
                    {
                        "tooltip": "Optional Civitai Auth connection; defaults to CIVITAI_API_TOKEN or stored OAuth login."
                    },
                ),
            },
        }


NODE_CLASS_MAPPINGS = {
    "CivitaiAceStepAudio": CivitaiAceStepAudio,
    "CivitaiAudioCaptioning": CivitaiAudioCaptioning,
    "CivitaiMiniMaxMusic3": CivitaiMiniMaxMusic3,
    "CivitaiTextToSpeechVllmOmniQwen3Base": CivitaiTextToSpeechVllmOmniQwen3Base,
    "CivitaiTextToSpeechVllmOmniQwen3CustomVoice": CivitaiTextToSpeechVllmOmniQwen3CustomVoice,
    "CivitaiTextToSpeechVllmOmniQwen3VoiceDesign": CivitaiTextToSpeechVllmOmniQwen3VoiceDesign,
    "CivitaiTextToSpeechVllmOmniOmnivoice": CivitaiTextToSpeechVllmOmniOmnivoice,
    "CivitaiTranscription": CivitaiTranscription,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "CivitaiAceStepAudio": "Civitai Ace Step Audio",
    "CivitaiAudioCaptioning": "Civitai Audio Captioning",
    "CivitaiMiniMaxMusic3": "Civitai MiniMax Music 3",
    "CivitaiTextToSpeechVllmOmniQwen3Base": "qwen3 / base",
    "CivitaiTextToSpeechVllmOmniQwen3CustomVoice": "qwen3 / customVoice",
    "CivitaiTextToSpeechVllmOmniQwen3VoiceDesign": "qwen3 / voiceDesign",
    "CivitaiTextToSpeechVllmOmniOmnivoice": "omnivoice",
    "CivitaiTranscription": "Civitai Transcription",
}
